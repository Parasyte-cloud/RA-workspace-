...
export default function InvestorInterestForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!form.consent) {
      setError('Please confirm that RideArrivo may contact you about this enquiry.')
      return
    }
    ...
    try {
      const response = await fetch(`${supabaseUrl}/functions/v1/ridearrivo-public-forms`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          action: 'submit_investor_interest',
      ...
      <main className="formsPage">
        <section className="formsShell formsSuccessShell">
          ...
          <div className="formsSuccess">
            ...
            <h1>Thank you for your interest in RideArrivo.</h1>
            <p>
              Your information has been securely received. A member of the RideArrivo team will
              review your enquiry and follow up using the contact details you provided.
            </p>
    ...
    <main className="formsPage">
      <section className="formsShell">
        ...
        <div className="formsHero">
          <div>
            <span className="formsEyebrow">INVESTOR & STAKEHOLDER INTEREST</span>
            <h1>Continue the conversation with RideArrivo.</h1>
            <p>
              Tell us how you would like to engage with RideArrivo. Your enquiry will be reviewed
              privately by our team.
            </p>
          </div>

          <aside>
            <span>PRIVATE FOLLOW-UP</span>
            <strong>Professional mobility. Built for scale.</strong>
            <p>
              Investment, strategic partnerships, information requests and meeting enquiries are
              welcome.
            </p>
          </aside>
        </div>
        ...
        <form className="formsCard" onSubmit={submit}>
          <div className="formsGrid">
            ...
            <label>
              ...
              <input
                ...
                onChange={event => setForm(current => ({ ...current, investmentRange: event.target.value }))}
              />
            </label>
          </div>
          ...
          <div className="formsActions">
            <button type="submit" disabled={busy}>
              {busy ? 'Submitting...' : 'Request follow-up'}
            </button>
            <small>Your submission is kept separate from ROOM 7 attendance and event analytics.</small>
          </div>
        </form>

        <footer className="formsFooter">
          <span>RideArrivo Limited</span>
          <span>Secure investor & stakeholder enquiries</span>
        </footer>
      </section>
    </main>
  )
