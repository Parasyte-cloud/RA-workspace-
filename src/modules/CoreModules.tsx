...
import EmployeeHrDetailsAdminPanel from '../components/EmployeeHrDetailsAdminPanel'
...
export function SupportModule(){
  ...
  return <section>
    ...
    <div className="grid2">
      ...
      <DataWorkbench table="support_quality_reviews" orderBy="reviewed_at" title="Service quality reviews" description="Review support interactions for quality, coaching and repeat-contact patterns." createLabel="Add review" fields={[{key:'case_reference',label:'Case reference',required:true},{key:'score',label:'Score (1-5)',type:'number'},{key:'review_notes',label:'Review notes',type:'textarea'},{key:'reviewed_by',label:'Reviewer',type:'employee'}]} columns={[{key:'case_reference',label:'Case'},{key:'score',label:'Score'},{key:'review_notes',label:'Review notes'},{key:'reviewed_by',label:'Reviewer',render:(row,employees)=>{const reviewerId=row.reviewed_by as string|undefined;if(!reviewerId){return 'Unassigned'}const reviewer=employees.find(candidate=>candidate.id===reviewerId);return reviewer?reviewer.full_name:'Unassigned'}}]}/>
...
export function PeopleModule({
  ...
  return <section>
    ...
    <div className="grid2">
      ...
      {canApproveLeave&&<EmployeeHrDetailsAdminPanel employees={employees}/>}
      {!operationsView&&<>
