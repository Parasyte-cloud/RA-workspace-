...
export function DataWorkbench({table,title,description,fields,columns,createLabel='Add record',orderBy='created_at'}:{table:string;title:string;description:string;fields:Field[];columns:Column[];createLabel?:string;orderBy?:string}){
 const [rows,setRows]=useState<Record<string,unknown>[]>([]),[form,setForm]=useState<Record<string,string>>({}),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[error,setError]=useState(''),[open,setOpen]=useState(false),[query,setQuery]=useState(''),[employees,setEmployees]=useState<{id:string;full_name:string;email:string}[]>([])
 ...
 useEffect(()=>{if(!supabase||!fields.some(f=>f.type==='employee'))return;void supabase.from('employee_profiles').select('id,full_name,email').eq('active',true).order('full_name').then(({data})=>setEmployees((data||[]) as {id:string;full_name:string;email:string}[]))},[table])
